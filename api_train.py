...
def val(opt, model, epoch, visualizer, epoch_iter, dataset_size, metric_name='F1_1'):
...
def train(
    ...
    --save_epoch_freq 1 
    --angle 15 
    --dataroot path-to-LEVIR-CD-train 
    --val_dataroot path-to-LEVIR-CD-val 
    --name LEVIR-CDFAp0 
    --lr 0.001 
    --model CDFA
    --SA_mode PAM
    ...
    opt.save_epoch_freq = save_epoch_freq
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
       ...
       # miou_current = val(opt, model)
        ...
        if epoch % opt.save_epoch_freq == 0:              # cache our model every <save_epoch_freq> epochs
            ...
            miou_current = val(opt, model, epoch, visualizer, epoch_iter, dataset_size, metric_name)
...
if __name__ == '__main__':
    name = 'test1'
    data_root = '/data/LEVIR_CD/train_new'
    val_data_root = '/data/LEVIR_CD/val_new'
    batch_size = 4
    train(name, data_root, val_data_root, 4)